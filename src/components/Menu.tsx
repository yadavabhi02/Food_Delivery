...
const Menu = () => {
  ...
  return (
    <div>
        {!open ?(
            <Image src="/temporary/open.png" alt="" width={20} height={20} onClick={()=>setOpen(true)}/>
        ) : (
            <Image src="/temporary/close.png" alt="" width={20} height={20} onClick={()=>setOpen(false)}/>
        )}  
        {open && (
            <div className="bg-red-400 text-white absolute left-0 top-24 w-full h-[calc(100vh-6rem)] flex flex-col items-center justify-center text-3xl z-10">
                <div className="flex flex-col gap-8 item-center text-center ">
                    {links.map(item=>(
